...
We are hoping to have a cloud instance of BarnebyLives(!) running soon. This is able to serve persons which have queries for collection in the Continental United States portions of Western North America (~CONUS). Essentially, you should be able to submit a query, and we will process it and send it back. 
...
**World Flora Online** provides the local taxonomic information required to perform much of the name matching. We utilize it to ensure that queries sent to **Plants of The World Online** are spelled correctly, in order to reduce the number of calls dispatched to through the API.

We will download WFO through the package **WorldFlora**. **WorldFlora** can be installed from CRAN

```{r, eval = F}
install.packages('WorldFlora')
```

Upon installation you will need to download the taxonomy for the package

```{r}
library(WorldFlora)
```

Once you have loaded the package you can access the taxonomy files like this.

```{r, eval = F}
WFO.download()
```

Once you have downloaded the taxonomy you can access it via

```{r, eval = F}
WFO.remember()
```

an alternative way to read it is to find the location of the data on your computer and load from there. Both us and the package use data.tables::fread function. 

```{r, eval = F}
cla <- data.table::fread(
  '../../Barneby_Lives-dev/taxonomic_data_raw/WFO_Backbone/classification.txt', na.strings = "")
...
The way that BarnebyLives matches spellings of names is very simple. We create three tables, one large table which contains all of the species in the WFO database, one moderately small table which contains all infraspecific taxa (varieties and subspecies), and a small table of all of the genera. 
...
families <- sort(distinct(cla$family))
families <- c(families, 'Hydrophyllaceae')
...
write.csv(sppLKPtab, '/hdd/Barneby_Lives-dev/taxonomic_data/species_lookup_table.csv', row.names = F)
write.csv(genLKPtab, '/hdd/Barneby_Lives-devs/taxonomic_data/genus_lookup_table.csv', row.names = F)
