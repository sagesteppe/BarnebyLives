...
While in theory all of the tasks in this script could be automated, doing so neglect the opinions of local taxonomic authorities. 
...
I preface this with the observation that a good deal of collectors and curators consider these data non-essential now. 
Regardless, if you are so inclined to gain access to a free google maps account these services are provided. 
...
The final process in the taxonomic step is to ensure that the spellings of any species noted in the *vegetation*, or *associates* fields are accurate. 
These *will not* then be submitted to Kew, as the fields could quickly overwhelm the API. 
...
Remove the collected species from associated species lists.
Many collectors include the focal collection when performing data entry.
...
After BarnebyLives verifies spelling, it submits names to Plants of the World Online to check for synonymy. 
BL will not automatically overwrite your submitted species - interpretation of results is important, especially as *spell_check* may occasionally mis-match material.
...
Write out the data for use in label generation, and as a master copy for exporting data for mass upload to a database. 
```{r, eval = F}
## Export Collections 
