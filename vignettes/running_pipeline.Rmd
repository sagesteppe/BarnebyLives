...
df <- googleway::google_directions(origin = "Reno, Nevada",
...
# identify the ORIGIN
sites <- dplr::bind_rows(sites)
origin <- places[ sf::st_nearest_feature(sites, places), c('NAME')] |>
  sf::st_drop_geometry()

# run query
directions <- googleway::google_directions(origin = origin,
                        destination = coords, key = SoS_gkey,
...
test_gq <- googs(data[c(1:3, 7:9, 15:17),], api_key = SoS_gkey)
