...
A default template will have some code that looks like this:
```{r, eval = F}
...
A modified template will look like the following. 
(**note: we'll put this spreadsheet in the directory soon**). 
...
Now we have the skeleton ready to receive data, but we will need to create another R script, which will be able to launch as many instances of `r`markdown::render` as we have labels we want to create. 
...
purrr::walk(
  ...
  ~ rmarkdown::render(
    input = "skeleton.Rmd",                      # template to populate
...
For example first we may subset our collections from a run of the BL pipeline 

and write these to be the data source which our skeleton will access

```{r example of modifying skeleton}

data('collection_examples')

data <- collection_examples |>
  #filter(Collection_number <= & Collection_number >= )
  dplyr::mutate(Coordinate_Uncertainty = '+/- 5m') 

write.csv(data, 'ExampleCollection.csv') # so accessible to our skeleton
```

next we will use the collection numbers to feed our chunk above. 
```{r}
...
It can be copied to your label generation directory using bash. 
...
**Note the `cp` command copies a file `from` to a `destiation`, in this case the current directory. 

the script can then be called from it's current location, and will create populate the `` directory with the combined labels for printing

```{sh, eval = F}
bash render_labels.sh 
```
