...
imagepath <- file.path( # shouldnt't need changed.  
  params$imagepath, paste0('map_', params$Collection_number, '.png')
  )

record <- collection_examples |>
  dplyr::filter(Collection_number == params$Collection_number) |>  
  sf::st_drop_geometry() # always remember no SF!!!
...
Veg: `r species_font(record[['Vegetation']])` `r associates_writer(record[['Associates']])` \par
