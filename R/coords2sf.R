...
coords2sf <- function(x, datum) {
  ...
  if (length(remove) > 0) {
    x <- x[-remove, ]
    cat('Error with row(s): ', remove, ' continuing without.')
  }
  rm(r, lat, long, remove)

  if (missing(datum)) {
    # identify datum information
    if (length(colnames(x)[grep('datum', colnames(x), ignore.case = T)]) == 1) {
      datum_name = colnames(x)[grep('datum', colnames(x), ignore.case = T)]
    } else {
      x$datum = 'WGS84'
    }
  }
  if (!exists('datum_name')) {
    datum_name = 'datum'
  }

  dat_check <- function(x) {
    if (grepl('nad.*27', ignore.case = T, x = x)) {
      x = "NAD27"
    } else if (grepl('nad.*83', ignore.case = T, x = x)) {
      x = 'NAD83'
    } else if (grepl('wgs', ignore.case = T, x = x)) {
      x = "WGS84"
    } else {
      x = 'WGS84'
    }
