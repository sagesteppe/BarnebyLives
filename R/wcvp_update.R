...
wcvp_update <- function(tax_dat_p) {
  ...
  if (length(list.files(file.path(tax_dat_p), pattern = 'wcvp.zip') == 1)) {
    unzip(
      zipfile = file.path(tax_dat_p, 'wcvp.zip'),
      files = "README_WCVP.xlsx",
      exdir = tax_dat_p
    )
    date_extracted <- gsub(
      'Extracted: ',
      ...
      as.character(
        suppressMessages(
          readxl::read_xlsx(path = file.path(tax_dat_p, 'README_WCVP.xlsx'))[
            7,
            1
          ]
        )
    ...
    date_uploaded <- unlist(regmatches(
      content,
      regexec(wcvp_pattern, content)
    ))[1]
    date_uploaded <- regmatches(
      date_uploaded,
      gregexpr(date_pattern, date_uploaded, perl = TRUE)
    )[[1]]
  ...
  if (dif > 14 | dif == 0) {
    # if two week or more difference,
    # re-download. 1st time i checked diff was 6 days

    orig_TO <- getOption("timeout") # save any local mods to timeout
    if (orig_TO < 420) {
      # allow users to set options in main env if need be.
      options(timeout = max(420, getOption("timeout")))
    } # 7 minute timeout
    download.file(
      url = paste0(url2wcvp, '/wcvp.zip'),
      destfile = file.path(tax_dat_p, 'wcvp.zip')
    )
    ...
    cat(crayon::green(
      'WCVP checklist does not appear to have been updated since last download, defaulting to current instance.'
    ))
  }
}
