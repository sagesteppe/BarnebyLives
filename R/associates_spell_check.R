...
associates_spell_check <- function(x, column, path) {
  if (missing(column)) {
    ...
    message(crayon::yellow(
      'Argument to `column` not supplied, defaulting to `Associates`'
    ))
  }
  ...
  infra_sppLKPtab <- read.csv(file.path(
    path,
    'infra_species_lookup_table.csv'
  )) |>
  ...
  for (y in seq_along(associates)) {
    ...
    for (i in seq_along(names2search)) {
      ...
      if (L == 1) {
        ...
        if (is.na(names2search[[i]])) {
          names2search[[i]] <- names2search[[i]]
        } else {
          genLKPsub <- genLKPtab[
            grepl(
              substr(names2search[[i]], 1, 2),
              genLKPtab$gGRP,
              ignore.case = TRUE
            ),
          ]
          names2search[[i]] <- paste(
            genLKPsub[
              which.min(
        ...
        if (nrow(sppLKPsub) == 0) {
          # if the two first letters don't match, we drop it to first letter.
          sppLKPsub <- sppLKPtab[
            grepl(
              substr(names2search[[i]], 1, 1),
              substr(sppLKPtab$gGRP, 1, 1),
              ignore.case = TRUE
            ),
          ]
        }
        ...
        # infra species search

        infra_sppLKPsub <- dplyr::filter(
          infra_sppLKPtab,
          gGRP == substr(names2search[[i]], 1, 2)
        )
        names2search[[i]] <- infra_sppLKPsub[
          which.min(
            stringdist::stringdist(
              names2search[[i]],
              infra_sppLKPsub$taxon_name,
              method = 'jw'
            )
          ),
          'taxon_name'
